...
# https://www.guru99.com/xpath-selenium.html help for writing certain xpaths
...
class Scraper:
  ...
  def arbitrage(self, oldMsgArray):
    calc = Calculations(self.grid, self.sportName)
    # Sets the message array to the calculations object so it know what arbitrage calls where made
    calc.setEmailMsgArray(oldMsgArray)
    calc.print()
    print(calc.anyArbitrage(self.grid))

    newMsgArray = calc.getEmailMsgArray()
    return newMsgArray
  ...
  def oddsSharkMMA(self):
    ...
    odds_top = driver.find_elements_by_xpath('//*[contains(@class,"op-block__cell-first-row")]/div')
    #                                         //div[@class="op-block__cell-first-row"]/div
    odds_top_list = [[]]
    counter = 0
    c = 0
    for top in odds_top:
    ...
    odds_bottom = driver.find_elements_by_xpath('//*[contains(@class,"op-block__cell-second-row")]/div')
    #                                            //div[@class="op-block__cell-second-row"]/div
    odds_bottom_list = [[]]
    counter = 0
    c = 0
    ...
    for book in books:
      if(book.get_attribute("aria-label") == book.get_attribute("aria-label").upper()):
        if(count1 != 0):
          ...
          count1+=1
    betting_sites.pop(len(betting_sites)-1)
...
#TODO ADD NCAAF BETTING LINES (COLLEGE FOOTBALL)
#TODO NEEDS TO BE UPDATED FOR NCAFF NAMES
  def oddsSharkNCAAF(self):
    chromedriver_autoinstaller.install()  # Check if the current version of chromedriver exists
                                          # and if it doesn't exist, download it automatically,
                                          # then add chromedriver to path
    driver = webdriver.Chrome()

    self.sportName = "College Football"
    website = "https://www.oddsshark.com/ncaaf/odds"
    driver.get(website)
    ...
    #TODO NEEDS TO BE UPDATED FOR NCAFF NAMES
    names = driver.find_elements_by_xpath('//div[@class="op-block__matchup-team-wrapper"]/div/a/span')
    #                                      //*[contains(@class,"op-block__matchup-team-wrapper")]/div/a/span    //*[contains(@class,"op-block__matchup-team-wrapper")]/div/a/span
    name_array = []
    name_array2 = []
    ...
    odds_top = driver.find_elements_by_xpath('//*[contains(@class,"op-block__cell-first-row")]/div')
    odds_top_list = [[]]
    counter = 1 #TODO double check to see if this works (not debugged)
    c = 0
    for top in odds_top:
      if(count == 0):
        tops = top.text
  ...
  def CaesarsMMA(self):
    ...
    for i in range(len(nameArr)):
      ...
      if(not any(nameArr[i] in sublist for sublist in self.grid)):
        ...
        self.grid[len(self.grid)-1].append(oddsArr[i])
  # ADDS ODDS DEPENDING ON THE DRAFTKINGS ARRAY
      else:
        # Adds the odds to the given player array if the player exsists on that array
        for j in range(len(self.grid)):
          # print(self.grid[j][0])
    ...
    # for i in range(len(nameArr)):
    #     # Checks if the name is in the not in the grid and adds the odds.
    #   if(any(nameArr[i] in sublist for sublist in self.grid)):
    #     #TODO this could be wrong
    #     self.grid.append([])
    #     self.grid[len(self.grid)-1].append(nameArr[i])
    #     # Adds a none because Draft kings doesn't have a list if this is checked
